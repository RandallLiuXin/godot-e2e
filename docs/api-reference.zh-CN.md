...
godot-e2e 捕获游戏侧的 `push_error`、`push_warning`、脚本运行时错误、shader 错误，以及（info verbosity 下）`print` / `printerr` 输出，并在 Python 侧暴露。需要 Godot 4.5+（参见 [`Logger`](https://docs.godotengine.org/en/4.5/classes/class_logger.html)）。
...
针对 Locator 的自动重试断言。每个 matcher 会在活跃游戏上轮询直到条件满足或超时，超时则抛 [`ExpectationFailedError`](#expectationfailederror)——它同时继承 `GodotE2EError` 和 `AssertionError`，pytest 会按普通断言失败渲染。
...
Locator 查询解析到 1 个或更多节点时通过。**不要求**唯一匹配——如果你需要正好 1 个，用 `to_satisfy(lambda l: l.count() == 1)`。
